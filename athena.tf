...
resource "null_resource" "execute_query" {
  provisioner "local-exec" {
    command = <<EOT
    $(aws sts assume-role --role-arn arn:aws:iam::${var.target_account_id}:role/aws_shafred_concourse --role-session-name aws-athena-run-query --query 'Credentials.[`export#AWS_ACCESS_KEY_ID=`,AccessKeyId,`#AWS_SECRET_ACCESS_KEY=`,SecretAccessKey,`#AWS_SESSION_TOKEN=`,SessionToken]' --output text | sed $'s/\t//g' | sed 's/#/ /g')
