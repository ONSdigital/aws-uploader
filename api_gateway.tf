...
resource "aws_apigatewayv2_route" "multipart_get" {
  #checkov:skip=CKV_AWS_309: Decision Auth is not required for this API
  api_id    = aws_apigatewayv2_api.api.id
  route_key = "GET /multipart-url"
  target    = "integrations/${aws_apigatewayv2_integration.multipart_get.id}"
}
...
resource "aws_apigatewayv2_route" "abort_multipart" {
  #checkov:skip=CKV_AWS_309: Decision Auth is not required for this API
  api_id    = aws_apigatewayv2_api.api.id
  route_key = "POST /abort-multipart"
  target    = "integrations/${aws_apigatewayv2_integration.abort_multipart.id}"
}
...
resource "aws_apigatewayv2_integration" "multipart_get" {
  api_id                 = aws_apigatewayv2_api.api.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.multiparturl.arn
  integration_method     = "GET"
  payload_format_version = "2.0"
}
...
resource "aws_apigatewayv2_integration" "complete_multipart" {
  api_id                 = aws_apigatewayv2_api.api.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.completeMultipartUpload.arn
  integration_method     = "POST"
  payload_format_version = "2.0"
}

resource "aws_apigatewayv2_integration" "abort_multipart" {
  api_id                 = aws_apigatewayv2_api.api.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.abortMultipartUpload.arn
  integration_method     = "POST"
  payload_format_version = "2.0"
}
