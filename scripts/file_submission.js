...
document.getElementById("form").addEventListener("submit", function (e) {
  ...
  console.log("DEBUG: Form submit handler running");
  ...
  if (fileOne.name.includes(ladCode) && !fileOne.name.match(patOne)) {
    console.log(
      "Extract File name does not follow the right pattern",
      fileOne.name,
    );
    fileOneErrorStyle();
  ...
  if (fileTwo.name.includes(ladCode) && !fileTwo.name.match(patTwo)) {
    console.log(
      "ManiFile name does not follow the right pattern",
      fileTwo.name,
    );
    fileTwoErrorStyle();
  ...
  const fileOneDate = fileOne.name
    .match(/\d{8}\.csv/)?.[0]
    ?.replace(".csv", "");
  const fileTwoDate = fileTwo.name
    .match(/\d{8}\.csv/)?.[0]
    ?.replace(".csv", "");
